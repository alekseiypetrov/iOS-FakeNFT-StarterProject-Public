...
final class ProductTableViewCell: UITableViewCell, ReuseIdentifying {
    ...
    private enum Constants {
        enum Sizes {
            static let ofRating = CGSize(width: 68.0, height: 12.0)
            static let ofDeleteButton = CGSize(width: 40.0, height: 40.0)
        }
        enum Heights {
            static let ofTitleAndPrice: CGFloat = 22.0
            static let ofStaticPriceLabel: CGFloat = 18.0
        }
    ...
    private lazy var productTitle: UILabel = {
        ...
        label.text = "Title"
        label.heightAnchor.constraint(equalToConstant: Constants.Heights.ofTitleAndPrice).isActive = true
        return label
    }()
    
    private lazy var productRating: UIImageView = {
        let imageView = UIImageView(image: UIImage(resource: .copRating0))
        imageView.heightAnchor.constraint(equalToConstant: Constants.Sizes.ofRating.height).isActive = true
        imageView.widthAnchor.constraint(equalToConstant: Constants.Sizes.ofRating.width).isActive = true
        return imageView
    }()
    ...
    private lazy var staticPriceLabel: UILabel = {
        ...
        label.heightAnchor.constraint(equalToConstant: Constants.Heights.ofStaticPriceLabel).isActive = true
        return label
    }()
